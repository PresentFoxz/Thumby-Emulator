...
def main():
    ...
    try:
        cam, player = initGame(worldChunks)
        WorldData.generateWorld(worldChunks, entities, int(player.x + player.box[0] + (player.box[2] // 2)), int(player.y + player.box[1] + (player.box[3] // 2)), False)
    ...
    while True:
        ...
        if game:
            ...
            if player.chunk[0] != new_x or player.chunk[1] != new_y:
                print(f"Pos: {player.chunk}")
                WorldData.generateWorld(worldChunks, entities, px, py, True)
